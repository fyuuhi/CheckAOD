...
cc -> cd(1);
  ...
  muon->SetTitle("sAddress vs pt residual ;muon residual;sAddress");
...
cc -> cd(4);
  ...
  TH2D *muon1= new TH2D("muon1","title",50,-3.2,3.2,5,-2,4);
  tree->Draw("sAddress:phiMS>>muon1",c_dR && c_barrel);
  muon1->SetTitle("sAddress vs phiMS ; phiMS ;sAddress");

  gPad->Update();

  TPaveStats *stn1 = (TPaveStats*) muon->FindObject("stats");
  stn1->SetX1NDC(0.7);
  stn1->SetX2NDC(0.9);
  stn1->SetY1NDC(0.8);
  muon1->Draw("COLZ");
